...
const ExitsPage = () => {
  ...
  const handleSerialEntry = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    const response = await inventoryService.getAll({serialNumber: inputs.serialNumber, soloStock: true}, ["id","partNumber", "descripcion", "serialNumber", "almacen", "sector", "ordenDeCompra", "facturaCompra", "fechaEntrada"]);
  ...
  const handleSubmit = async () => {
    ...
    if(response.status === 'OK') {
      ...
      alert(response.message, response.information);
      setIsLoading(false);
    }
  }
    ...
    <MainLayout>
      ...
      <FormFields tabs={tabs} renderInputs={renderInputs} activeTab={activeTab} handleChange={(tab) => handleTabChange(tab)} />
        {/* <div className={styles.tabs}>
          <FormTab title="Datos Salida" isActive={activeTab === "DATOS"} onClick={() => handleTabChange("DATOS")}/>
